    ...
    fn setup(&self, ctx: PluginContext) -> Result<Box<OutputInstance>> {
        ...
        let accept = socket.incoming()
            ...
            .for_each(move |(socket, addr)| {
                ...
                hello_connections.lock().map_err(report_and_discard)?.insert(addr, tx);
                ...
                let reader =
                    io::read_exact(reader, vec![0; 1]).map(|_| ()).map_err(report_and_discard);

                let conn = reader.select(socket_writer);

                handle.spawn(conn.then(move |_| {
                    info!("disconnect: {}", addr);
                    bye_connections.lock().map_err(report_and_discard)?.remove(&addr);
                    Ok(())
                }));
...
impl OutputInstance for SnoopOutputInstance {
    fn feed(&self, sample: &Sample) -> Result<()> {
        ...
        for (_addr, tx) in c.iter_mut() {
            let _r = tx.send(out.clone())
                .map_err(|_| ErrorKind::Message("failed to feed".to_owned()))?;
        }
